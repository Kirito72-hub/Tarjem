...
export const LogsView: React.FC = () => {
  ...
  const getStepIcon = (type: LogStep['type']) => {
    switch (type) {
      case 'SUCCESS':
        return <CheckCircle className="text-green-500/80" size={14} />
      case 'ERROR':
        return <XCircle className="text-red-500/80" size={14} />
      case 'WARNING':
        return <AlertTriangle className="text-amber-500/80" size={14} />
      default:
        return <div className="w-3.5 h-3.5 rounded-full bg-gray-700/50 border border-gray-600" />
    }
  }
            ...
            {logs.map((log) => (
              ...
              >
                ...
                >
                  ...
                  <div className="text-xs font-mono text-gray-600">{formatTime(log.timestamp)}</div>
                </div>
                  ...
                  <div className="border-t border-white/5 bg-[#0F111A]/30 p-4 pl-12 flex flex-col gap-2">
                    {log.steps.map((step) => (
                      <div key={step.id} className="flex gap-3 text-sm group">
                        ...
                        <div className="flex-1 min-w-0">
                          <div
                            className={`text-gray-300 ${step.type === 'ERROR' ? 'text-red-400' : ''}`}
                          >
                            {step.message}
                          </div>
                          {step.detail && (
