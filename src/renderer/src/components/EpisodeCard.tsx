...
import { CheckCircle2, AlertCircle, Loader2, FileVideo, Search, Merge, Hash, Square, CheckSquare, Trash2, FileType, Captions, Download } from 'lucide-react';
...
export const EpisodeCard: React.FC<EpisodeCardProps> = ({ episode, onToggle, onRemove, onDownload }) => {
  ...
  const isProcessing = episode.stage !== ProcessingStage.COMPLETED && episode.stage !== ProcessingStage.ERROR && episode.stage !== ProcessingStage.IDLE && episode.stage !== ProcessingStage.REVIEW;
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {episode.stage === ProcessingStage.IDLE || episode.stage === ProcessingStage.REVIEW || isSubtitle ? (
      ...
      {episode.stage === ProcessingStage.REVIEW && episode.searchResults && episode.searchResults.length > 0 && (
        <div className="bg-[#1C212E]/50 border-b border-white/5 px-8 py-4 animate-in fade-in slide-in-from-top-2 duration-200">
          ...
          <div className="flex flex-col gap-1">
            {episode.searchResults.map((result) => (
              <div key={result.id} className="flex items-center gap-4 py-2 px-3 rounded hover:bg-white/5 transition-colors group/result">
                <div className="flex-1 min-w-0 flex items-center gap-2">
                  <Captions size={14} className="text-gray-500" />
                  <span className="text-sm text-gray-300 truncate font-medium">{result.filename}</span>
                </div>
                ...
                <div className="w-24 text-xs text-gray-500 flex items-center gap-1">
                  <span className={`w-1.5 h-1.5 rounded-full ${result.source === 'OpenSubtitles' ? 'bg-blue-500' : 'bg-green-500'}`}></span>
                  {result.source}
                </div>
