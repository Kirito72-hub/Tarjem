...
import { Upload, Trash2, Search, CheckSquare, Square, Settings, Merge, FileSearch, FileVideo, Captions, Globe, Download, Star, Loader2 } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    const activeCount = episodes.filter(e => e.stage !== 'COMPLETED' && e.stage !== 'ERROR' && e.stage !== 'IDLE').length;
    ...
    const canStart = episodes.some(e => e.selected && e.stage === 'IDLE' && (activeTab === 'FILE_MATCH' || e.fileType === 'VIDEO'));
        ...
        <div className="flex-1 h-[calc(100vh-32px)] overflow-hidden flex flex-col bg-[#0F111A]">
                    ...
                    <div>
                        ...
                        {activeTab !== 'WEB_SEARCH' && (
                            <div className="flex items-center gap-4 text-sm text-gray-500">
                                {activeCount > 0 ? (
                                    <span className={`flex items-center gap-1.5 ${activeTab === 'FILE_MATCH' ? 'text-purple-400' : 'text-blue-400'}`}>
                                        <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${activeTab === 'FILE_MATCH' ? 'bg-purple-500' : 'bg-blue-500'}`}></div>
                ...
                <div className="flex gap-3">
                    ...
                    {activeTab !== 'WEB_SEARCH' && (
                        <>
                            ...
                            <button
                                onClick={() => activeTab === 'FILE_MATCH' ? videoInputRef.current?.click() : generalInputRef.current?.click()}
                    ...
                    {episodes.length === 0 ? (
                        <div className="flex-1 relative p-4" onDrop={handleDrop} onDragOver={handleDragOver}>
                            ...
                            >
                                ...
                                <div className="text-center">
                                    <p className="text-sm font-medium text-gray-300 group-hover:text-white">Drop Episodes Here to Auto-Match</p>
                                    <p className="text-xs text-gray-600">Analyzes file hash to find exact subtitles</p>
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="flex flex-col h-full">
                            ...
                            <div className="flex items-center px-8 py-3 bg-[#161B22] border-b border-white/5 text-xs font-medium text-gray-500 uppercase tracking-wider select-none shrink-0">
                                <button onClick={onToggleAll} className="w-8 flex justify-center text-gray-500 hover:text-white transition-colors mr-4">
                                    {allSelected ? <CheckSquare size={16} className="text-purple-500" /> : <Square size={16} />}
                                </button>
                                <div className="flex-1">File Name</div>
                                <div className="w-64">Status</div>
                                <div className="w-32 px-4 text-right flex items-center justify-end gap-3">
                                    <span>Size</span>
                                    <button onClick={onRemoveSelected} disabled={selectedCount === 0} className={`transition-colors ${selectedCount > 0 ? 'text-gray-400 hover:text-red-400 cursor-pointer' : 'text-transparent pointer-events-none'}`}>
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="overflow-y-auto custom-scrollbar p-0 flex-1">
                                {episodes.map(ep => (
                                    <EpisodeCard
                                        ...
                                        onDownload={(result) => onDownloadEpisodeSubtitle && onDownloadEpisodeSubtitle(ep.id, result)}
                                    />
                                ))}
                            </div>
                        </div>
            ...
            {activeTab === 'WEB_SEARCH' && (
                <div className="flex-1 flex flex-col min-h-0 relative">
                    {/* Search Input Area */}
                    <div className={`transition-all duration-300 ${searchResults.length > 0 || isSearchingWeb ? 'px-8 py-4 bg-[#161B22] border-b border-white/5' : 'absolute inset-0 flex items-center justify-center px-4'}`}>
                        <form
                ...
                <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 flex flex-col min-h-0 border-b border-white/5">
                        <div className="px-6 py-2 bg-[#161B22] border-b border-white/5 flex items-center justify-between shrink-0">
                            ...
                            {videoFiles.length > 0 && (
                                <button onClick={() => videoInputRef.current?.click()} className="text-xs text-gray-500 hover:text-white flex items-center gap-1">
                                    <Upload size={12} /> Add
                                </button>
                            )}
                        </div>

                        <div className="flex-1 overflow-y-auto custom-scrollbar relative bg-[#0F111A]" onDrop={handleDrop} onDragOver={handleDragOver}>
                            {videoFiles.length === 0 ? (
                                <div className="absolute inset-0 p-4">
                                    ...
                                    >
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm font-medium text-gray-300 group-hover:text-white">Drop Episodes Here</p>
                                ...
                                videoFiles.map(ep => (
                                    <EpisodeCard key={ep.id} episode={ep} onToggle={() => onToggleEpisode(ep.id)} onRemove={() => onRemoveEpisode(ep.id)} />
                                ))
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col min-h-0">
                        <div className="px-6 py-2 bg-[#161B22] border-b border-white/5 flex items-center justify-between shrink-0">
                            ...
                            {subtitleFiles.length > 0 && (
                                <button onClick={() => subtitleInputRef.current?.click()} className="text-xs text-gray-500 hover:text-white flex items-center gap-1">
                                    <Upload size={12} /> Add
                                </button>
                            )}
                        </div>
                            ...
                            {subtitleFiles.length === 0 ? (
                                <div className="absolute inset-0 p-4">
                                    ...
                                    >
                                        ...
                                        <div className="text-center">
                                            <p className="text-sm font-medium text-gray-300 group-hover:text-white">Drop Subtitles Here</p>
