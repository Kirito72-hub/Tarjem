            ...
            <div className="max-w-3xl mx-auto space-y-8">
                                        ...
                                        {filteredLanguages.length === 0 ? (
                                            ...
                                            filteredLanguages.map((lang) => (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-md text-sm transition-colors ${language === lang.code
                ...
                <div className="bg-[#1C212E] rounded-xl border border-white/5 p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-150">
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1.5">
                            <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">OpenSubtitles API Key</label>
                            <input
                                type="text"
                                placeholder="Enter your OpenSubtitles.com API Key"
                                onChange={(e) => {
                                    if (window.api && window.api.settings) {
                                        window.api.settings.set('opensubtitles_api_key', e.target.value);
                                    }
                                }}
                                ...
                                ref={(input) => {
                                    if (input && window.api && window.api.settings) {
                                        window.api.settings.get('opensubtitles_api_key').then(val => {
                            ...
                            <p className="text-xs text-gray-500">
                                Required for subtitle searching. Get one at <a href="#" onClick={() => window.electron?.ipcRenderer.send('open-external', 'https://opensubtitles.com')} className="text-purple-400 hover:underline">opensubtitles.com</a>.
                            </p>
                        </div>

                        <div className="space-y-1.5 pt-4 border-t border-white/5">
                            <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">SubDL API Key</label>
                            <input
                                type="text"
                                placeholder="Enter your SubDL API Key"
                                onChange={(e) => {
                                    if (window.api && window.api.settings) {
                                        window.api.settings.set('subdl_api_key', e.target.value);
                                    }
                                }}
                                className="w-full bg-[#0F111A] border border-white/10 rounded-lg px-3 py-2 text-sm text-gray-300 focus:outline-none focus:border-purple-500/50 transition-colors"
                                ref={(input) => {
                                    if (input && window.api && window.api.settings) {
                                        window.api.settings.get('subdl_api_key').then(val => {
                                            if (val) input.value = val;
                                        });
                                    }
                                }}
                            />
                            <p className="text-xs text-gray-500">
                                Required for SubDL. Get one at <a href="#" onClick={() => window.electron?.ipcRenderer.send('open-external', 'https://subdl.com')} className="text-purple-400 hover:underline">subdl.com</a>.
                            </p>
                        </div>
                    </div>
                </div>
